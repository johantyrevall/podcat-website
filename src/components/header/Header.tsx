...
function Header() {
  ...
  return (
    <header
      className={`sticky top-0 z-50 bg-white transition-transform duration-200 ${
    ...
    >
      <div className="flex flex-row items-center p-6 md:p-10 h-20 md:h-25">
        ...
        <div className="flex flex-row items-center flex-grow">
          <Link to="/" className="text-3xl font-semibold hover:text-gray-800 transition-colors mr-8">
            Podcat
          </Link>
          <nav className="hidden md:flex flex-row gap-x-8 items-center">
            {menuItems.map(({ label, to, id }) => (
              <Link
                key={id}
                to={to}
        ...
        >
          ...
          {menuItems.map(({ label, to, delay, id }) => (
            <Link
              key={id}
              to={to}
              className={`hover:text-gray-800 transform transition-all duration-500 ${
          ...
          <div className="absolute bottom-20">
            <a
              ...
              className={`text-black hover:underline hover:text-gray-800 transform transition-all duration-500 ${
                mobileMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
              }`}
